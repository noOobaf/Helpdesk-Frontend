...
const sampleStateData: State[] = [
  ...
  { key: "3", countryName: "India", stateName: "Himachal", description: "-" },
];
...
const StateInformationDashboard: React.FC = () => {
  ...
  // State for managing state data
  const [stateData, setStateData] = useState<State[]>(sampleStateData);
  const [editingRecord, setEditingRecord] = useState<State | null>(null);
  ...
  const handleDelete = (key: string) => {
    confirm({
      ...
      onOk() {
        setStateData((prevData) => prevData.filter((item) => item.key !== key));
      },
    });
  };

  // Table columns definition moved inside the component
  const tableColumns: ColumnsType<State> = [
    {
      title: (
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRecord) {
        ...
        setStateData((prevData) => [...prevData, newRecord]);
      }
  ...
  const plusMenuItems = [
    { key: "add-state", label: "Add State", onClick: showModal },
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      >
        <Content style={{ padding: "24px" }}>
          ...
          <Modal
            title={editingRecord ? "Edit State" : "Add State"}
            open={isModalVisible}
...
export default StateInformationDashboard;
